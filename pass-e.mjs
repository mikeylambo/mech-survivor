...
replace("let runCoins=0,finalBossSpawned=false,activeWorld=0,eliteStage=0,choiceMode='upgrade',blessingStage=0;","let runCoins=0,finalBossSpawned=false,activeWorld=0,eliteStage=0,choiceMode='upgrade',blessingStage=0,director=null,eliteKills=0;",'director runtime state');
replace("eliteStage=0;blessingStage=0;choiceMode='upgrade';runCoins=0;finalBossSpawned=false;","eliteStage=0;blessingStage=0;choiceMode='upgrade';eliteKills=0;director=null;runCoins=0;finalBossSpawned=false;",'director reset state');
replace("updateBuild();state='play';hideScreens();hud.classList.remove('hidden');buildUI.classList.remove('hidden');last=performance.now()}","director=createRunDirector({world:activeWorld,seed:String(activeWorld)+'-'+Date.now()});ensureDirectorHud();updateDirectorHud();updateBuild();state='play';hideScreens();hud.classList.remove('hidden');buildUI.classList.remove('hidden');last=performance.now()}",'director reset init');
replace("if(e.t==='elite')spawnCache(e.x,e.y,'elite');if(e.t==='boss')","if(e.t==='elite'){eliteKills++;spawnCache(e.x,e.y,'elite')}if(e.t==='boss')",'elite kill tracking');
...
replace("function update(dt){elapsed+=dt;player.invuln=","function update(dt){elapsed+=dt;tickRunDirector();player.invuln=",'director update tick');
replace("synergies:[...player.synergies],blessings:[...player.blessings],corruption:player.corruption}}","synergies:[...player.synergies],blessings:[...player.blessings],corruption:player.corruption,director:{completed:director?.completed||0,failed:director?.failed||0,history:director?.history||[],phase:directorPhase(elapsed).id}}}",'director run summary');
replace("stop(){state='title';hud.classList.add('hidden');buildUI.classList.add('hidden');enemies=[];shots=[];enemyShots=[];gems=[]}","stop(){state='title';hud.classList.add('hidden');buildUI.classList.add('hidden');document.querySelector('#director-objective')?.classList.add('hidden');enemies=[];shots=[];enemyShots=[];gems=[]}",'director stop cleanup');
