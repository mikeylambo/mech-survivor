export function initVFXLab(fx,getAnchor=()=>({x:0,y:0})){
  ...
  addEventListener('keydown',e=>{if(e.code==='F8'){e.preventDefault();root.style.display=root.style.display==='none'?'block':'none'}else if(root.style.display!=='none'&&e.code==='KeyV'&&!e.repeat)preview()});
  ...
  window.SLUVFX={fx,play:(id,params={})=>{const p=getAnchor()||{x:0,y:0};return fx.play(id,{x:p.x,y:p.y,...params})},toggle:()=>root.style.display=root.style.display==='none'?'block':'none'};
}
