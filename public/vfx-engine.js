...
export function createVFXEngine(options={}){
  ...
  const maxActive=options.maxActive??1400;
  ...
  function play(id,params={}){const recipe=recipes.get(id);if(!recipe)return false;const p={x:0,y:0,x2:null,y2:null,dx:1,dy:0,intensity:1,...params};for(const m of recipe.modules||[]){const at=num(m.at,p,0);if(at>0)scheduled.push({time:at,module:m,params:p});else spawnModule(m,p)}return true}
  function spawnModule(m,p){
    ...
    if(m.type==='particles'){
      const count=scaledCount(Math.max(1,num(m.count,p,8)),intensity),base=Math.atan2(num(p.dy,p,0),num(p.dx,p,1)),spread=num(m.spread,p,TAU),speed0=num(m.speedMin,p,45),speed1=num(m.speedMax,p,180),life0=num(m.lifeMin,p,.18),life1=num(m.lifeMax,p,.5);
      for(let i=0;i<count;i++){const a=m.omni?rand(0,TAU):base+rand(-spread/2,spread/2),s=rand(speed0,speed1)*(.75+.35*intensity),life=rand(life0,life1);active.push({kind:'particle',x:num(p.x,p,0)+rand(-num(m.jitter,p,0),num(m.jitter,p,0)),y:num(p.y,p,0)+rand(-num(m.jitter,p,0),num(m.jitter,p,0)),vx:Math.cos(a)*s,vy:Math.sin(a)*s,life,max:life,size0:num(m.sizeMin,p,1),size1:num(m.sizeMax,p,4),drag:num(m.drag,p,4),gravity:num(m.gravity,p,0),color:color(m.color,p,'#fff'),shape:m.shape||'square',stretch:num(m.stretch,p,1),alpha:num(m.alpha,p,1)});}
      return;
    }
    ...
    if(m.type==='ring')active.push({kind:'ring',x:num(p.x,p,0),y:num(p.y,p,0),life,max:life,r0:num(m.r0,p,4),r1:num(m.r1,p,50)*(.8+.2*intensity),width0:num(m.width0,p,4),width1:num(m.width1,p,1),color:color(m.color,p,'#fff'),alpha:num(m.alpha,p,1),ease:m.ease||'outCubic'});
      ...
      const x=num(p.x,p,0),y=num(p.y,p,0),x2=p.x2==null?x+num(p.dx,p,1)*num(m.length,p,180):num(p.x2,p,x),y2=p.y2==null?y+num(p.dy,p,0)*num(m.length,p,180):num(p.y2,p,y);
      active.push({kind:'beam',x,y,x2,y2,life,max:life,width:num(m.width,p,5),core:num(m.core,p,.35),color:color(m.color,p,'#fff'),coreColor:color(m.coreColor,p,'#fff'),alpha:num(m.alpha,p,1),pulse:num(m.pulse,p,0)});
    }
    else if(m.type==='bolt'){
  ...
  function draw(ctx){
    ...
    for(const o of active){const t=clamp(1-o.life/o.max),fade=1-t,fn=ease[o.ease]||ease.linear,e=fn(t);ctx.globalAlpha=fade*o.alpha;ctx.strokeStyle=o.color;ctx.fillStyle=o.color;ctx.shadowColor=o.color;
      ...
      else if(o.kind==='beam'){const pulse=1+(o.pulse?Math.sin((1-t)*35)*o.pulse:0);ctx.lineCap='round';ctx.shadowBlur=18;ctx.lineWidth=o.width*pulse;ctx.strokeStyle=o.color;ctx.beginPath();ctx.moveTo(o.x,o.y);ctx.lineTo(o.x2,o.y2);ctx.stroke();ctx.shadowBlur=7;ctx.lineWidth=Math.max(1,o.width*o.core);ctx.strokeStyle=o.coreColor;ctx.stroke()}
      else if(o.kind==='bolt'){const dx=o.x2-o.x,dy=o.y2-o.y,len=Math.hypot(dx,dy)||1,nx=-dy/len,ny=dx/len;ctx.lineCap='round';ctx.shadowBlur=14;ctx.lineWidth=o.width;ctx.beginPath();ctx.moveTo(o.x,o.y);for(let i=1;i<o.segments;i++){const q=i/o.segments,decay=Math.sin(Math.PI*q),n=Math.sin((o.seed+i*17.31))*o.jitter*decay*(.55+.45*fade);ctx.lineTo(o.x+dx*q+nx*n,o.y+dy*q+ny*n)}ctx.lineTo(o.x2,o.y2);ctx.stroke()}
    }
