...
function renderShop(){const grid=$('#shop-grid');grid.innerHTML='';for(const item of shopItems){const level=save.upgrades[item.id]||0,price=cost(item,level),maxed=level>=item.max,card=document.createElement('article');card.className=`shop-card ${maxed?'maxed':''}`;card.innerHTML=`<div class="tier">${item.id==='mythic'?'ULTIMATE LONG-TERM GOAL':'PERMANENT POWER'} · LV ${level}/${item.max}</div><h3>${item.name}</h3><p>${item.desc}</p><button ${maxed||save.coins<price?'disabled':''}>${maxed?'MAXIMUM':`◈ ${price}`}</button>`;card.querySelector('button').onclick=()=>{if(save.coins<price||maxed)return;save.coins-=price;save.upgrades[item.id]=level+1;persist();renderShop();focusButton(0)};grid.append(card)}renderWallets()}
